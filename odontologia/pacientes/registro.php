...
$servicio_redirect = isset($_GET['servicio']) ? $_GET['servicio'] : '';
$redirect = isset($_GET['redirect']) ? $_GET['redirect'] : '/odontologia/panel_paciente.php';
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $anio_nac = date("Y", strtotime($fecha_nacimiento));
    $anio_actual = date("Y");
    if ($anio_nac < 1930 || $anio_nac > $anio_actual) {
        $mensaje = "⚠️ La fecha de nacimiento debe estar entre 1930 y $anio_actual.";
    }
    elseif (!preg_match('/^\d{8}$/', $cedula)) {
        $mensaje = "⚠️ La cédula uruguaya debe tener exactamente 8 números.";
    } else {
        ...
        if ($resultado->num_rows > 0) {
            ...
            $stmt1 = $conn->prepare("INSERT INTO pacientes (nombre, apellido, email, telefono, cedula, fecha_nacimiento) VALUES (?, ?, ?, ?, ?, ?)");
            $stmt1->bind_param("ssssss", $nombre, $apellido, $email, $telefono, $cedula, $fecha_nacimiento);
            ...
            if ($stmt1->execute()) {
                ...
                if ($stmt2->execute()) {
                    ...
                    if (!empty($servicio_redirect)) {
                        header("Location: /odontologia/pacientes/solicitar_turno.php?servicio=" . urlencode($servicio_redirect));
                        exit();
                    } else {
...
<div class="container mt-5">
    ...
    <form method="POST" class="card p-4 shadow mx-auto" style="max-width: 500px;">
        ...
        <div class="mb-3"><label>Cédula (Uruguaya, 8 dígitos):</label>
            <input type="text" name="cedula" class="form-control" maxlength="8" pattern="\d{8}" title="Debe tener 8 números" required>
        </div>
        <div class="mb-3"><label>Fecha de nacimiento:</label>
            <input type="date" name="fecha_nacimiento" class="form-control" min="1930-01-01" max="<?= date("Y-m-d") ?>" required>
        </div>
