...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!$cedula || !preg_match('/^\d{8}$/', $cedula)) $errores[] = "Cédula inválida: debe tener exactamente 8 números";
    ...
    if (empty($errores)) {
        ...
        if ($resCedula->num_rows > 0) {
            ...
            $stmt = $conn->prepare("INSERT INTO Pacientes (nombre, apellido, email, telefono, fecha_nacimiento, cedula) VALUES (?, ?, ?, ?, ?, ?)");
            $stmt->bind_param("ssssss", $nombre, $apellido, $email, $telefono, $fecha_nacimiento, $cedula);

            if ($stmt->execute()) {
                $id_paciente = $stmt->insert_id;

                $contrasena_hash = password_hash($contrasena, PASSWORD_DEFAULT);
                $stmtUser = $conn->prepare("INSERT INTO Usuarios (nombre_usuario, contrasena, rol, id_paciente) VALUES (?, ?, ?, ?)");
                $stmtUser->bind_param("sssi", $nombre_usuario, $contrasena_hash, $rol, $id_paciente);

                if ($stmtUser->execute()) {
                    $mensaje = "Paciente y usuario creados con éxito. Ahora puede ingresar con el nombre de usuario y contraseña.";
                } else {
                    $errores[] = "Error al crear usuario: " . $conn->error;
                }
                $stmtUser->close();
            } else {
                $errores[] = "Error al guardar paciente: " . $conn->error;
            }
...
<h3 class="mb-3">Crear Nuevo Paciente</h3>
...
<form method="post">
  ...
  <div class="mb-3">
    ...
    <input type="text" class="form-control" id="cedula" name="cedula" value="<?= $_POST['cedula'] ?? '' ?>" maxlength="8" pattern="\d{8}" title="Debe tener 8 números" required>
  </div>
