...
if (isset($_GET['id'])) {
    $turnoId = intval($_GET['id']);
  
    $stmt = $conn->prepare("SELECT * FROM turnos WHERE id = ? AND id_pacientes = ?");
    $stmt->bind_param("ii", $turnoId, $pacienteId);
    $stmt->execute();
...
if ($turno['estado'] !== 'pendiente') {
    ...
    <head>
        <meta charset='UTF-8'>
        <title>Error - No se puede modificar</title>
            ...
            <div class='card shadow' style='max-width: 500px; margin: 0 auto;'>
                <div class='card-body text-center'>
                    <div class='alert alert-warning'>
                        <h4>🔒 No se puede modificar este turno</h4>
                        <p class='mb-3'>Solo puedes modificar turnos que estén en estado <strong>PENDIENTE</strong>.</p>
                        <p>Estado actual: <span class='badge bg-";
    
    switch ($turno['estado']) {
        case 'confirmado':
            echo "success'>Confirmado";
            break;
        case 'cancelado':
            echo "danger'>Cancelado";
            break;
        default:
            echo "secondary'>" . htmlspecialchars($turno['estado']);
    }
    
    echo "</span></p>
                        <hr>
                        <p class='mb-0'><small>Si necesitas modificar un turno confirmado, contacta a la clínica al <strong>+598 092 434 321</strong></small></p>
                    </div>
                    <a href='../panel_paciente.php' class='btn btn-primary'>🔙 Volver al Panel</a>
                </div>
            </div>
        </div>
...
<head>
  ...
  <style>
    ...
    .turno-info { background: #e7f3ff; padding: 15px; border-radius: 8px; margin-bottom: 20px; border-left: 4px solid #0d6efd; }
  </style>
</head>
<body>
...
<div class="card">
  ...
  <div class="turno-info">
    <small class="text-muted">Estado del turno:</small>
    <h6 class="mb-0"><span class="badge bg-warning">⏳ Pendiente</span></h6>
  </div>
