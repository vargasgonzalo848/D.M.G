...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!$cedula || !preg_match('/^\d{8}$/', $cedula)) $errores[] = "La cédula es obligatoria y debe tener 8 números";
    ...
    if (empty($errores)) {
        $stmt = $conn->prepare("UPDATE Dentistas SET nombre = ?, apellido = ?, cedula = ?, email = ?, telefono = ?, fecha_nacimiento = ? WHERE id = ?");
        $stmt->bind_param("ssssssi", $nombre, $apellido, $cedula, $email, $telefono, $fecha_nacimiento, $id);
        if ($stmt->execute()) {
            $mensaje = "Dentista actualizado correctamente";
            $dentista['nombre'] = $nombre;
            $dentista['apellido'] = $apellido;
            $dentista['cedula'] = $cedula;
...
<form method="post">
  ...
  <div class="mb-3">
    ...
    <input type="text" class="form-control" id="cedula" name="cedula" maxlength="8" pattern="\d{8}" value="<?= htmlspecialchars($dentista['cedula']) ?>" required>
  </div>
  <div class="mb-3">
