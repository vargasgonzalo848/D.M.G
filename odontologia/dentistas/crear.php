...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($errores)) {
        $stmt = $conn->prepare("INSERT INTO Dentistas (nombre, apellido, cedula, email, telefono, fecha_nacimiento) VALUES (?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("ssssss", $nombre, $apellido, $cedula, $email, $telefono, $fecha_nacimiento);
...
<form method="post">
  ...
  <div class="mb-3">
    ...
    <input type="text" class="form-control" id="cedula" name="cedula" maxlength="8" pattern="\d{8}" value="<?= $_POST['cedula'] ?? '' ?>" required>
  </div>
  <div class="mb-3">
