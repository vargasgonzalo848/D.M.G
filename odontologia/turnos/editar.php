...
// Si el turno está confirmado, no se puede modificar
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Validar fecha: solo próxima semana
    $hoy = strtotime(date('Y-m-d'));
    $lunes_prox_semana = strtotime("next monday", $hoy);
    $domingo_prox_semana = strtotime("next sunday +7 days", $hoy); // Domingo de la próxima semana
    $fecha_timestamp = strtotime($fecha_turno);

    if ($fecha_timestamp < $lunes_prox_semana || $fecha_timestamp > $domingo_prox_semana) {
        $errores[] = "Solo puedes seleccionar fechas de la próxima semana.";
    }

    // Validar hora: intervalos de 30 minutos y horario correcto
    if ($hora_turno) {
